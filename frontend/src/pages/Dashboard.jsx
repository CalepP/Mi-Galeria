...
export default function Dashboard() {
  ...
  const [nuevaGaleria, setNuevaGaleria] = useState({ nombre: '', fecha: '', password: '', musica: 'Sin música' })
  const { fotografo, logout } = useAuth()
  ...
  const cargarGalerias = async () => {
    try {
      const { data } = await axios.get(`${API}/galerias`)
      setGalerias(data)
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const crearGaleria = async () => {
    if (!nuevaGaleria.nombre) return
    try {
      const { data } = await axios.post(`${API}/galerias`, nuevaGaleria)
      setGalerias(prev => [data, ...prev])
      setModalNueva(false)
      setNuevaGaleria({ nombre: '', fecha: '', password: '', musica: 'Sin música' })
    } catch (err) {
      console.error(err)
    }
  }
    ...
    <div className="min-h-screen" style={{ background: '#f5ede0' }}>
      ...
      <div className="fixed left-0 top-0 h-full w-16 flex flex-col items-center py-6 gap-6 z-50 border-r" style={{ background: '#2a1f14', borderColor: '#3d2b1a' }}>
        <div className="w-10 h-10 rounded-full border-2 flex items-center justify-center mb-4" style={{ borderColor: '#c9a96e' }}>
          <span className="font-serif italic text-sm" style={{ color: '#c9a96e' }}>
            {fotografo?.nombre?.[0] || 'F'}
          </span>
        </div>
        {[
        ...
        <button onClick={handleLogout} className="w-10 h-10 flex items-center justify-center text-lg" style={{ color: '#6b4c2a' }} title="Salir">↩</button>
      </div>
      ...
      <div className="ml-16">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-4 py-2 border" style={{ borderColor: '#e8d5bc', background: '#f5ede0' }}>
              ...
              <input type="text" placeholder="Buscar cliente..." value={busqueda}
                ...
                className="outline-none bg-transparent text-sm w-40" style={{ color: '#2a1f14' }}/>
            </div>
            ...
            <div className="relative">
              ...
              {menuPerfil && (
                <div className="absolute right-0 top-12 w-48 border shadow-lg z-50" style={{ background: '#fdfaf6', borderColor: '#e8d5bc' }}>
                  <div className="px-4 py-3 border-b" style={{ borderColor: '#e8d5bc' }}>
                    <p className="text-sm font-medium" style={{ color: '#2a1f14' }}>{fotografo?.nombre}</p>
                    <p className="text-xs" style={{ color: '#8a6e52' }}>{fotografo?.email}</p>
                  </div>
                  <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-sm hover:opacity-70" style={{ color: '#2a1f14' }}>
                    Cerrar sesión
                  </button>
                </div>
              )}
        ...
        <div className="px-8 py-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              { label: 'Galerías', valor: galerias.length, icon: '⊞' },
              { label: 'Total de fotos', valor: galerias.reduce((a, g) => a + (g.fotos?.length || 0), 0), icon: '◎' },
              { label: 'Activas', valor: galerias.filter(g => g.visible).length, icon: '↓' },
              { label: 'Con música', valor: galerias.filter(g => g.musica !== 'Sin música').length, icon: '♪' },
            ].map((stat, i) => (
              <div key={i} className="p-5 border flex items-center gap-4" style={{ background: '#fdfaf6', borderColor: '#e8d5bc' }}>
            ...
            <div className="flex items-center gap-2">
              {['todas', 'activa', 'borrador'].map(f => (
                <button key={f} onClick={() => setFiltro(f)}
                  ...
                  style={{ background: filtro === f ? '#2a1f14' : 'transparent', color: filtro === f ? '#c9a96e' : '#8a6e52', borderColor: filtro === f ? '#2a1f14' : '#e8d5bc' }}>
                  {f}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              {['grid', 'lista'].map(v => (
                <button key={v} onClick={() => setVista(v)}
                  ...
                  style={{ background: vista === v ? '#2a1f14' : 'transparent', color: vista === v ? '#c9a96e' : '#8a6e52', borderColor: '#e8d5bc' }}>
                  {v === 'grid' ? '⊞' : '≡'}
                </button>
              ))}
            </div>
          ...
          {cargando ? (
            <div className="text-center py-20">
              <p className="font-serif text-xl italic" style={{ color: '#8a6e52' }}>Cargando galerías...</p>
            </div>
          ) : vista === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {galeriasFiltradas.map(g => (
                <div key={g.id} className="border overflow-hidden hover:shadow-md transition-all group" style={{ background: '#fdfaf6', borderColor: '#e8d5bc' }}>
                  <div className="h-44 flex items-center justify-center relative overflow-hidden" style={{ background: '#2a1f14' }}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all" style={{ background: 'rgba(42,31,20,0.7)' }}>
                      <div className="flex gap-2">
                        <button onClick={() => navigate(`/galeria/${g.id}`)} className="px-3 py-1.5 text-xs tracking-widest uppercase" style={{ background: '#c9a96e', color: '#2a1f14' }}>Editar</button>
                        <button onClick={() => window.open(`/g/${g.slug}`, '_blank')} className="px-3 py-1.5 text-xs tracking-widest uppercase border" style={{ borderColor: '#c9a96e', color: '#c9a96e' }}>Ver</button>
                      </div>
                    </div>
                    {g.musica !== 'Sin música' && (
                  ...
                  <div className="p-4">
                    <h3 className="font-serif text-lg mb-1" style={{ color: '#2a1f14' }}>{g.nombre}</h3>
                    <p className="text-xs mb-3" style={{ color: '#8a6e52' }}>{g.fecha || 'Sin fecha'}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs" style={{ color: '#8a6e52' }}>◎ {g.fotos?.length || 0} fotos</span>
                      <button onClick={() => copiarLink(g.slug)} className="text-xs" style={{ color: '#c9a96e' }}>🔗 Link</button>
                    </div>
              ...
              <button onClick={() => setModalNueva(true)}
                className="border-2 border-dashed min-h-64 flex flex-col items-center justify-center gap-3 hover:opacity-70 transition-all"
            ...
            <div className="flex flex-col gap-3">
              {galeriasFiltradas.map(g => (
                <div key={g.id} className="p-5 border flex items-center justify-between gap-4" style={{ background: '#fdfaf6', borderColor: '#e8d5bc' }}>
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 flex items-center justify-center" style={{ background: '#f5ede0' }}>
                      <CameraIcon size={24}/>
                    </div>
                    <div>
                      <h3 className="font-serif text-lg" style={{ color: '#2a1f14' }}>{g.nombre}</h3>
                      <div className="flex gap-3 mt-1">
                        <span className="text-xs" style={{ color: '#8a6e52' }}>{g.fecha || 'Sin fecha'}</span>
                        <span className="text-xs" style={{ color: '#c9a96e' }}>♪ {g.musica}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <button onClick={() => navigate(`/galeria/${g.id}`)} className="text-xs px-4 py-2 border tracking-widest uppercase" style={{ borderColor: '#c9a96e', color: '#2a1f14' }}>Editar</button>
                    <button onClick={() => copiarLink(g.slug)} className="text-xs" style={{ color: '#c9a96e' }}>🔗</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="w-full max-w-md border p-8" style={{ background: '#fdfaf6', borderColor: '#e8d5bc' }}>
            ...
            <div className="space-y-5">
              {[
                ...
                { label: 'Contraseña (opcional)', key: 'password', type: 'password', placeholder: 'Dejar vacío para pública' },
              ].map(field => (
                <div key={field.key}>
                  <label className="block text-xs tracking-widest uppercase mb-2" style={{ color: '#8a6e52' }}>{field.label}</label>
                  <input type={field.type} placeholder={field.placeholder} value={nuevaGaleria[field.key]}
                    onChange={e => setNuevaGaleria({ ...nuevaGaleria, [field.key]: e.target.value })}
                    className="w-full px-0 py-2 border-b outline-none bg-transparent text-sm"
                    style={{ borderColor: '#c9a96e', color: '#2a1f14' }}/>
                </div>
              ))}
              <div>
                <label className="block text-xs tracking-widest uppercase mb-2" style={{ color: '#8a6e52' }}>Música</label>
                <select value={nuevaGaleria.musica} onChange={e => setNuevaGaleria({ ...nuevaGaleria, musica: e.target.value })}
                  className="w-full py-2 border-b outline-none bg-transparent text-sm" style={{ borderColor: '#c9a96e', color: '#2a1f14' }}>
                  {['Sin música', 'Perfect — Ed Sheeran', 'Thinking Out Loud', 'A Thousand Years — Christina Perri', 'Can\'t Help Falling in Love'].map(m => (
                    <option key={m}>{m}</option>
                  ))}
                </select>
              </div>
              <button onClick={crearGaleria}
                className="w-full py-3 text-xs tracking-widest uppercase mt-4 hover:opacity-80 transition-all"
                style={{ background: '#2a1f14', color: '#c9a96e' }}>
                Crear galería →
              </button>
            </div>
